...
| 扫描行数     | 多    | **少**   |
#### 聚簇索引、覆盖索引
##### 聚簇索引（主键索引）Clustered Index
索引的叶子节点直接存放完整的数据行、索引结构 = 数据本身
在 InnoDB 中：
- 主键（PRIMARY KEY）默认就是聚簇索引
- 如果没有主键：
    - 找唯一非空索引作为聚簇索引
    - 如果没有，MySQL 自动生成隐藏字段作为聚簇索引
##### 非聚簇索引（二级索引）Secondary Index
只保存索引字段 + 主键值，查询数据使用回表机制，查询id在查询出表，返回信息，查询变两次 B+Tree 查询
##### 覆盖索引（Covering Index）
查询字段全部存在于索引中，不需要回表，比如查询的内容索引值都包含就是覆盖索引
![](static/Snipaste_2026-08-03_10-03-09.png)

| 类型    | 叶子节点存什么  | 是否包含数据 | 是否需要回表 |
| ----- | -------- | ------ | ------ |
| 聚簇索引  | 完整数据行    | 是      | 否      |
| 非聚簇索引 | 索引字段+主键  | 否      | 通常需要   |
| 覆盖索引  | 查询字段全部包含 | 部分数据   | 否      |
![索引类型结构图](./static/Snipaste_2026-08-03_09-37-11.png)

#### redis索引
MySQL：
> 索引 = 为了快速定位数据，MySQL额外维护的一棵数据结构（B+Tree）

Redis：
> 索引 = 通常指通过 **Key 的组织方式、数据结构、额外建立的数据结构** 来实现快速查询。
Redis 本身没有像 MySQL 那样自动维护二级索引。
---
### MySQL 排查线程、锁、索引
