...
一个事务内，同一条数据读取两次，结果不一样。**同一条数据，值变化了**
![](static/repeatable.png)

###### 幻读（Phantom Read）
一个事务两次查询同一个范围，返回的记录数量不同。
![](static/phantomRead.png)

|问题|关注点|例子|
|---|---|---|
|脏读|数据内容|读到了别人未提交的数据|
|不可重复读|同一行数据变化|同一条记录值改变|
|幻读|数据数量变化|查询结果多了一行|

|隔离级别|脏读|不可重复读|幻读|
|---|---|---|---|
|READ_UNCOMMITTED|❌可能|❌可能|❌可能|
|READ_COMMITTED|✅解决|❌可能|❌可能|
|REPEATABLE_READ|✅解决|✅解决|部分解决|
|SERIALIZABLE|✅解决|✅解决|✅解决|
脏读： 看到了别人没提交的数据 （数据真假问题） 
不可重复读： 同一条数据前后不一样 （数据修改问题） 
幻读： 同一个条件查询，数量变了 （数据新增删除问题）
##### timeout（事务超时时间）
...
## str字符串分隔改为数组
```java
String idStr = this.matGoodsMapper.getToolGoodsIds(key, tid);  
Set<String> ids = new HashSet<>();  
if (idStr != null && !idStr.isEmpty()) {  
    ids.addAll(Arrays.stream(idStr.split(","))  
            .map(String::trim)  
            .filter(s -> !s.isEmpty())  
...
| RoundingMode  | 中文名称     | 规则                    | 正数示例 `1.235 → 保留2位` | 负数示例 `-1.235 → 保留2位` | 常见场景              |
| ------------- | -------- | --------------------- | ------------------- | -------------------- | ----------------- |
| `UP`          | 远离 0 舍入  | 只要有非 0 小数就进位          | `1.24`              | `-1.24`              | 很少使用              |
| `DOWN`        | 向 0 舍入   | 直接截断，不进位              | `1.23`              | `-1.23`              | 截断数量              |
| `CEILING`     | 向正无穷     | 向更大的方向舍入              | `1.24`              | `-1.23`              | 上取整               |
| `FLOOR`       | 向负无穷     | 向更小的方向舍入              | `1.23`              | `-1.24`              | 下取整               |
| `HALF_UP`     | **四舍五入** | `5` 及以上进位             | **`1.24`**          | **`-1.24`**          | **最常用，金额/BOM/数量** |
| `HALF_DOWN`   | 五舍六入     | 正好 `5` 不进位，超过 `5` 才进位 | `1.23`              | `-1.23`              | 较少使用              |
| `HALF_EVEN`   | 银行家舍入    | 正好 `5` 时向偶数位靠拢        | `1.24`              | `-1.24`              | 金融/统计             |
| `UNNECESSARY` | 不允许舍入    | 必须能够精确表示，否则抛异常        | 异常                  | 异常                   | 强制精确计算            |
...
| 10b：再加一件跨库同步    | 同步 ~3700 ms / 异步 ~200 ms | 把手机号、客户等级等字段同步到**另一张库表**；同步版耗时线性增长，异步版几乎不变 |
...
| 特性                | 现实场景                                                              |
| ----------------- | ----------------------------------------------------------------- |
| Work Queue 工作队列   | 批量导出发票/对账单、批量生成报表、图片转码、批量推送——多个消费者分摊，快的人多干           |
| fanout 广播         | 配置变更通知所有节点刷新本地缓存、订单创建后同时通知库存/积分/风控/物流                            |
| direct 精确路由       | 日志分级：error 单独一队走告警，info/warning 一队只落库；按业务类型分流（订单/售后/财务）        |
| topic 通配符         | 事件总线：`order.pay.success`、`order.cancel`、`user.register`，各系统按前缀订阅（`order.#`、`user.*`） |
| 死信 DLX            | 第三方回调失败（支付回调、物流回传）重试 N 次后落库人工处理；对账差错单；消息格式非法要单独捞出来排查   |
| 延迟队列              | 下单 30 分钟未支付自动取消、15 分钟未评价提醒、7 天自动确认收货、拼团到期未成团退款、预约提醒、工单超时升级 |
| Confirm 三保险       | 支付成功通知、发券、扣库存这类「少一条就出事故」的消息；配合定时任务扫描补偿表做兜底                     |
| RPC               | 把 MQ 当轻量 RPC：调打印服务打单、调编号服务生成单号、跨服务查客户信用额度（免去同步 HTTP 调用的强耦合） |
| 异步解耦              | 注册后发邮件/短信、把用户资料同步到另一张库表（场景 10/10b）、下单后写埋点/推送/生成对账            |
| 削峰                | 秒杀下单先入队、批量导入 Excel 分片入队、夜间跑批                                          |
